...
case class CartDto(userId: String,
	...
	def setList(itemList: List[ProductOptionItemDto]): CartDto =
		new CartDto(
			userId, cartId, name, productId, thumbnail, price, quantity, addedDate, itemList
		)
...
object CartDto {
	...
	implicit val getCartResult = GetResult { r =>
		CartDto(r.nextString(), r.nextInt(), r.nextString(),r.nextInt(), r.nextString(),r.nextInt(), r.nextInt(), r.nextDate())
	}
}
